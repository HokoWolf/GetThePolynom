...
{
    public partial class ValuesForFault : Form
    {
        Menu menu;
        double[,] values;

        public ValuesForFault(Menu menu)
        {
            InitializeComponent();
            this.menu = menu;
        }

        private void FaultChoice_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void ValuesForFault_Load(object sender, EventArgs e)
        {
        ...
        {
            if (txtPreviousFunction.Text == "")
                MessageBox.Show("Не введена оригінальна функція!");
            else
            {
                string function = txtPreviousFunction.Text.ToLower().Replace("pi", "Pi");
                FaultByValues faultByValues = new FaultByValues(this, values, function);
                faultByValues.Show();
                this.Enabled = false;
            }
        }
